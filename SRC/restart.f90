...
! (just in case when some parameterization initializes and needs boundary points)
        ...
        write(65)  &
         ...
         rho, rhow, bet, sstxy, sttxy, rank, nx, ny, nz, irecc !, plocation, det1 !,&
!            fluxbt,fluxbq,fluxbu,fluxbv,&
!            fluxtt,fluxtq,fluxtu,fluxtv,&
!            fluxlt,fluxlq,fluxlv,fluxlw,&
!            fluxrt,fluxrq,fluxrv,fluxrw,&
!            fluxqt,fluxqq,fluxqu,fluxqw,&
!            fluxht,fluxhq,fluxhu,fluxhw
        close(65)
        if(masterproc) then
           write(66) version, &
            ...
            dowallx, dowally,dowallsflux,dowallmflux, doperpetual, doseasons, &
            docup, docolumn, soil_wetness, dodynamicocean, ocean_type, &
            delta_sst, depth_slab_ocean, Szero, deltaS, timesimpleocean, &
            pres0, ug, vg, fcor, fcorz, tabs_s,tabs_t,tabs_w, z0, fluxt0, fluxq0, tau0, &
            ts_b, ts_t, ts_n, ts_s, ts_w, ts_e, &
            soil_b, soil_t, soil_n, soil_s, soil_w, soil_e, &
            dos_b, dos_t, dos_n, dos_s, dos_w, dos_e, &
            ...
            cem, les, ocean, land, sfc_flx_fxd, sfc_tau_fxd, cns_flx, &
            ...
            ndet_rtd,ndet_sonicu,ndet_sonicv,ndet_sonicw,ndet_licor, &
            ndet_thermistors, ndet_virtual ,pdet,idet,ndetout
            close(66)
        end if
        ...
        read(65)  &
         u, v, w, t, p, qv, qcl, qci, qpl, qpi, dudt, dvdt, dwdt, &
         tracer, micro_field, sgs_field, sgs_field_diag, z, pres, prespot, presi, &
         rho, rhow, bet, sstxy, sttxy, rank1, nx1, ny1, nz1, irecc !, plocation, det1 !,&
!            fluxbt,fluxbq,fluxbu,fluxbv,&
!            fluxtt,fluxtq,fluxtu,fluxtv,&
!            fluxlt,fluxlq,fluxlv,fluxlw,&
!            fluxrt,fluxrq,fluxrv,fluxrw,&
