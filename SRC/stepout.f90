...
if(mod(nstep,nsave3D).eq.0.and.nstep.ge.nsave3Dstart.and.nstep.le.nsave3Dend ) then
  ...
  if(qnmax(1).ge.qnsave3D) then 
    call write_fields3D()
    ! Less frequent outputs to conserve space
    ! ST 07/11/2019
    !if(mod(nstep/nsave3D,10)) then
!MO    call write_fields3D_micro()
    if(mod(nstep,nsave3D_micro).eq.0) call write_fields3D_micro()   
    !end if
  end if
endif
call t_stopf ('3D_out')
...
!--------------------------------------------------------
 if(masterproc) then
    ...
    if (dos_b) write(6,*) 'Bottom surface T (K)=', ts_b
    if (dos_t) write(6,*) 'Top surface T (K)=', ts_t
    if (dos_n) write(6,*) 'North wall surface T (K)=', ts_n
    if (dos_s) write(6,*) 'South wall surface T (K)=', ts_s
    if (dos_w) write(6,*) 'West surface T (K)=', ts_w
    if (dos_e) write(6,*) 'East surface T (K)=', ts_e
 ...
 call average2XY_MPI_print('shf_bot_avg:',fluxbt*cp*rhow(1),1,nx,1,ny,1)
 call average2XY_MPI_print('shf_top_avg:',fluxtt*cp*rhow(nzm),1,nx,1,ny,1)
 call average2XY_MPI_print('lhf_bot_avg:',fluxbq*lcond*rhow(1),1,nx,1,ny,1)
 call average2XY_MPI_print('lhf_top_avg:',fluxtq*lcond*rhow(nzm),1,nx,1,ny,1)
 call average2XY_MPI_print('lhf_left_avg:',fluxlq*lcond*rhow(nzm),1,ny,1,nzm,1)
 call average2XY_MPI_print('lhf_right_avg:',fluxrq*lcond*rhow(nzm),1,ny,1,nzm,1)
 call average2XY_MPI_print('lhf_front_avg:',fluxqq*lcond*rhow(nzm),1,nx,1,nzm,1)
 call average2XY_MPI_print('lhf_back_avg:',fluxhq*lcond*rhow(nzm),1,nx,1,nzm,1)
 ...
 total_water_before = total_water_before/float(nx_gl*ny_gl*ncycle)
 total_water_after = total_water_after/float(nx_gl*ny_gl*ncycle)
 total_water_evap = total_water_evap/float(nx_gl*ny_gl*ncycle)
 total_water_prec = total_water_prec/float(nx_gl*ny_gl*ncycle)
 total_water_ls = total_water_ls/float(nx_gl*ny_gl*ncycle)
