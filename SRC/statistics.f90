...
! END UW ADDITIONS
!========================================================================
! Bulk outputs for cloud chamber
! Removal of biases due to the presence of the cloud chamber walls at
! top,bottom and lateral surfaces
! Added by Subin Thomas on 11/12/2018
! Modified by Fan Yang on 11/21/2020
!========================================================================
! bulk water vapor mixing ratio, temperature, TKE, relative humidity
real, dimension(nzm) :: qv_b,tabs_b,tke_b,relh_b
! bulk liquid water content, droplet number concentration, aerosol number
! concentration, droplet removal rate
real, dimension(nzm) :: qc_b,nc_b, na_b, ncr_b
! bulk droplet mean size, standard deviation, droplet effective mean radius, relative dispersion
real, dimension(nzm) :: rmean_b, rsig_b, reff_b, reldis_b
! bulk spatial variance of supersaturation, spatial variance of T, spatial
! variance of water vapor mixing ratio
real, dimension(nzm) :: sigma_s_b, sigma_tabs_b, sigma_qv_b
! bulk eddy dissipation rate
real, dimension(nzm) :: epsilon_b
...
!    counter_b = 0.
!*********************************************************************
    !nx_b = nx - offset_f - offset_b
    !ny_b = ny - offset_f - offset_b
    !*********************************************************************
    ! If you ever change the offset_f or offset_b change the following 
    ! Calculate the total counter_b 
    ! Accomplished by uncommenting the lines related to counter_b and
    ! total_counter_b, and total_counter_b is replaced as the denominator for
    ! factor_xy_b
    !*********************************************************************
    !factor_xy_b = 16./float(14*228)
    !*********************************************************************
    !do k=1,nzm
    !    qv_b(k)=0.
    !    tabs_b(k)=0.
    ...
    !nx_b = xeb - xsb + 1
    !ny_b = yeb - ysb + 1
    !factor_xy_b = 1.0 / float(nx_b*ny_b)
    ...
    do k=zsb,zeb
        do i=xsb,xeb
            do j=ysb,yeb
	            qv_b(k) =qv_b(k)+qv(i,j,k)
                !tabs_b(k)=tabs_b(k)+tabs(i,j,k)+gamaz(k)
                tabs_b(k)=tabs_b(k)+tabs(i,j,k)
                ...
                qc_b(k) = qc_b(k) + qcl(i,j,k)
!*********************************************************************
! Uncomment for calculating the bulk for changed offsets
!*********************************************************************
...
!*********************************************************************
    ...
    !print*, rank, tabs(10,10,10),pres(10), qv(10,10,10), qsatw(tabs(10,10,10)), qv(10,10,10)/qsatw(tabs(10,10,10),pres(10))
    !print *,rank,qv_b(16)*factor_xy_b,tabs_b(16)*factor_xy_b,relh_b(16)*factor_xy_b,factor_xy_b
    !print *,rank,qc_b(16)*factor_xy_b
!*********************************************************************
! Uncomment for calculating the bulk for changed offsets
!*********************************************************************
!    counter_b = counter_b/(zeb-zsb+1)
!    call task_sum_integer(counter_b,total_counter_b,1)
!    print *,rank,tabs_b(16)*factor_xy_b,relh_b(16)*factor_xy_b*100

	!---------------------------------------------------------
	! Output bulk values calculated to stat file
    ! Subin Thomas 11/13/2018
    ! Modified Fan Yang 11/21/2020
    !---------------------------------------------------------
    call hbuf_put('BQV',qv_b,1.e3*factor_xy_b)
	call hbuf_put('BTABS',tabs_b,factor_xy_b)
	call hbuf_put('BTKE',tke_b,factor_xy_b)
	call hbuf_put('BRELH',relh_b,100.*factor_xy_b)
    call hbuf_put('BQC',qc_b,1.e3*factor_xy_b)
!*********************************************************************
!========================================================================
...
!  v(nxi,nyi,nzi)=1.0
!  w(nxi,nyi,nzi)=2.0
!  tabs(nxi,nyi,nzi)=3.0
!  det1tmp(1)=0.5*(u(nxi,nyi,nzi)+u(nxi+1,nyi,nzi))
!  det1tmp(2)=0.5*(v(nxi,nyi,nzi)+v(nxi,nyi+1,nzi))
!  det1tmp(3)=0.5*(w(nxi,nyi,nzi)+w(nxi,nyi,nzi+1))
!  det1tmp(4)=tabs(nxi,nyi,nzi)
! end if
! if (i.eq.1) then
! end if
!end do 
...
! top,bottom and lateral surfaces
! Added by Subin Thomas on 11/12/2018
! Variables of interest DISSIPS,QV,RELH,TABS,TKE,
!===================================================
